...
class AddFoodToDietScreen extends StatefulWidget {
  final FoodDetailItem food;
  final String diet;

  const AddFoodToDietScreen({Key? key, required this.food, required this.diet}) : super(key: key);
...
class _AddFoodToDietScreenState extends State<AddFoodToDietScreen> {
  ...
  num energy = 0.0;
  var _isLoading = true;
  List<FoodPortionDetail> foodPortionDetail = [];

  @override
  void initState() {
    super.initState();
    fetchData(widget.food.Food_Id);
  }

  Future<void> fetchData(int foodId) async {
    await httpService.getPortionDetails(foodId).then((value) {
      foodPortionDetail.addAll(value);
      foodPortionDetail.forEach((element) {
        if (selectedSize == "small" && element.portionSize == "S") {
      ...
      body: !_isLoading
        ...
        Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(
              widget.food.Food_Desc.toUpperCase(),
              style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 18),
            ),
            Text(
              "50.0g - " + energy.round().toString() + "cal",
                      ...
                      ListWheelScrollView.useDelegate(
                        ...
                        childDelegate: ListWheelChildBuilderDelegate(
                          ...
                          builder: (context, index) {
                            return WheelTile(selectedQuantity == quantity[index] ? Colors.black : Colors.black45, quantity[index]);
                          },
                        ),
                      ),
                    ],
                    ...
                    children: [
                      ...
                      ListWheelScrollView.useDelegate(
                        ...
                        onSelectedItemChanged: (index) {
                          setState(() {
                            ...
                            foodPortionDetail.forEach((element) {
                              if (selectedSize == "small" && element.portionSize == "S") {
                                energy = element.Energy;
                              } else if (selectedSize == "medium" && element.portionSize == "M") {
                                energy = element.Energy;
                              } else if (selectedSize == "large" && element.portionSize == "L") {
                                energy = element.Energy;
                              }
                            });
                          });
                        ...
                        childDelegate: ListWheelChildBuilderDelegate(
                          ...
                          builder: (context, index) {
                            return WheelTile(selectedSize == size[index] ? Colors.black : Colors.black45, size[index]);
                          },
                        ),
                      ),
                    ],
            ...
            Row(
              ...
              children: [
                InkWell(
                  onTap: () {
                    Navigator.pop(context, [widget.food, selectedQuantity, selectedSize, (num.parse(selectedQuantity) * energy).round().toString()]);
                  },
                  child: Container(
